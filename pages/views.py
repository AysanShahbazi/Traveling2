...
def contact(request):
    """Contact page view - ذخیره در دیتابیس (نه ایمیل)"""
...
def admin_user_edit(request, user_id):
    ...
    if request.method == 'POST':
        try:
            ...
            if profile:
                profile.user_type = request.POST.get('user_type', 'customer')
...
def manage_payments(request):
    ...
    payments_list = Payment.objects.select_related('booking').all().order_by('-created_at')

    # آمار
    total_count = payments_list.count()
    success_count = payments_list.filter(status='success').count()
    failed_count = payments_list.filter(status='failed').count()
    pending_count = payments_list.filter(status='pending').count()

    total_revenue = sum([p.amount for p in payments_list if p.status == 'success'])
...
def view_payment(request, pk):
    ...
    return render(request, 'admin/payments/view.html', {'payment': payment})
...
def delete_payment(request, pk):
    ...
    payment.delete()
    messages.success(request, f'Payment {ref} deleted successfully!')
    return redirect('pages:manage_payments')


@admin_required
def view_message(request, pk):
    """نمایش یک پیام + علامت خوانده‌شده"""
    from .models import ContactMessage
    ...
    return render(request, 'admin/messages/view.html', {'message_obj': msg})
...
def delete_message(request, pk):
    ...
    return redirect('pages:manage_messages')
